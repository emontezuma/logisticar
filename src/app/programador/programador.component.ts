...
export class ProgramadorComponent implements OnInit {
  ...
  ) 
  {
    ...
    if (this.tiempoFalta && this.datos.tiempo != -1)
    {
      ...
      this.datos.boton1STR =  "Aceptar (" + this.tiempoFalta + ")";      
    }
  ...
  validar(id: number)
  {
    if (this.clave != "Cronos20" + this.servicio.fecha(1, "", "HHmmmmHH"))
    {
      ...
      mensajeCompleto.mensaje = "La clave de programador no coincide (" + this.servicio.fecha(1, "", "HHmmmmHH") + ")";
    ...
    mensajeCompleto.mensaje = "Acceso como programador permitido";
    mensajeCompleto.tiempo = 2000;
    this.servicio.mensajeToast.emit(mensajeCompleto);
