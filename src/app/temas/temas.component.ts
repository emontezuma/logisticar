  ...
  validar(id: number)
  {
    if (id == 1)
    {
      const respuesta = this.dialogo.open(DialogoComponent, {
        width: "420px", panelClass: 'dialogo_atencion', data: { titulo: "GUARDAR TEMA", mensaje: "Esta acción AFECTARÁ a todos los usuarios que usen el tema: <strong>" + this.reporte + "</strong><br><br><strong>¿Desea continuar con la operación?</strong><br><br>Los colores no especificados serán sustituidos por el color blanco (#FFFFFF)", id: 0, accion: 0, tiempo: 0, botones: 2, boton1STR: "Guardar", icono1: "in_seleccionado", boton2STR: "Cancelar", icono2: "i_cancelar", icono0: "i_eliminar" }
      });
        ...
        if (result)
        {
          if (result.accion == 1) 
          {
            ...
            this.servicio.consultasBD(campos).subscribe( resp =>
            {
              ...
              this.bot1 = false;
              mensajeCompleto.mensaje = "El tema se ha guardado satisfactoriamente";
...
buscarColores()
{
  ...
  this.servicio.consultasBD(campos).subscribe( resp =>
  {
    if (resp.length > 0)
    {
      ...
      let mensajeCompleto: any = [];
      mensajeCompleto.clase = "snack-normal";
      mensajeCompleto.mensaje = "Se recuperan los colores del tema";
    ...
    else
    {
      ...
      mensajeCompleto.mensaje = "El tema no existe";
      mensajeCompleto.tiempo = 2000;
      this.servicio.mensajeToast.emit(mensajeCompleto);
    }
    this.bot1 = false;
...
{
  const respuesta = this.dialogo.open(DialogoComponent, {
    width: "420px", panelClass: 'dialogo_atencion', data: { titulo: "ELIMINAR REGISTRO", mensaje: "Esta acción ELIMINARÁ PERMANENTEMENTE el tema: <strong>" + this.reporte + "</strong> y no estará disponible<br><br><strong>¿Desea continuar con la operación?</strong>", id: 0, accion: 0, tiempo: 0, botones: 2, boton1STR: "Eliminar", icono1: "in_seleccionado", boton2STR: "Cancelar", icono2: "i_cancelar", icono0: "i_eliminar" }
  });
  respuesta.afterClosed().subscribe(result => 
  {
    if (result)
    {
      if (result.accion == 1) 
      {
        ...
        this.servicio.consultasBD(campos).subscribe( resp =>
        {
          ...
          this.validar03 = false;
          this.validar04 = false;
          mensajeCompleto.mensaje = "El tema se ha eliminad satisfactoriamente";
          mensajeCompleto.tiempo = 3000;
          this.servicio.mensajeToast.emit(mensajeCompleto);
          setTimeout(() => {
            this.txtT1.nativeElement.focus();  
